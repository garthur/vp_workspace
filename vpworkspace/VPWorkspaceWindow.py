...
# adding modules to path for imports we wont actually use it in this 
# file, but just in case files from adjacent directories are necessary
TOPDIR_PATH = os.path.dirname(os.path.abspath(__file__))
sys.path.append(TOPDIR_PATH)

# import modules for function eval
import sympy as sym
from sympy.parsing.sympy_parser import parse_expr
# some special functions need to be imported for eval purposes
# maybe this can be handled in a later function??
from sympy import cos, acos, sin, asin, tan, atan, cot, sec, csc, pi, ln, log, sqrt

# import self-written modules for manipulations
from source.vectorClass import Vector
from source.polynomialClass import PolynomialMultiVar

# import test module for running tests on startup
import tests
...
class VPWorkspaceWindow(EventBasedAnimationClass):
	...
	def checkFilesHandler(self):
		...
		try:
			tests.pathExistsTest.checkAllFiles()
		except:
	...
	def initAnimation(self):
		...
		self.root.protocol("WM_DELETE_WINDOW", self.savePopupAndQuit)
		...
		self.helpIndex = 0
		# initialize variable to help with file I/O
		self.recentPath = None
		# initialize right-click popup menu
		self.initPopupMenu()

	def runClassTests(self):
		# runs tests on the vector and polynomial classes to make
		# sure that there are no issues in the class
		pass
...
# the two functions below can probably be combined

	def saveWorkspace(self, quitInProgress = False):
		# turns workspace into a string, gets path from tkFileDialog
		contents = self.makeWorkspaceReprString()
		# make sure we are not continually saving empty
		# workspaces at different times
		if (contents == ""):
			print "This workspace is empty. Saving terminated."
			return
		# create a folder if none exists yet
		if (not os.path.exists("Saved Workspaces")):
			os.makedirs("Saved Workspaces")
		# now proceed to save
		if(quitInProgress and self.recentPath != None):
			filename = self.recentPath
		else:
			filename = tkFileDialog.asksaveasfilename(initialdir="Saved Workspaces", 
				defaultextension=".txt")
			self.recentPath = filename
